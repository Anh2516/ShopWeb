...
const Wallet = () => {
  ...
  const handleConfirmTransfer = async () => {
    ...
    try {
      ...
      window.dispatchEvent(
        new CustomEvent("newTopupRequest", {
          detail: {
            userId: user?.id,
            amount: pendingTopup.amount,
          },
      ...
      window.dispatchEvent(
        new CustomEvent("pendingCountsUpdate", {
  ...
  const qrUrl =
    pendingTopup && user?.customer_code
      ? `https://img.vietqr.io/image/mbbank-2516999999999-compact2.png?amount=${encodeURIComponent(
          pendingTopup.amount
        )}&addInfo=${encodeURIComponent(
          user.customer_code
        )}&accountName=DANG%20TUAN%20ANH`
      : null;
    ...
    <div className="main-content">
      <div className="container wallet-container">
          ...
          <div className="wallet-card">
            ...
            <form onSubmit={handleTopUp}>
              ...
              <div className="form-note">
                <p>
                  Hệ thống sẽ tạo QR VietQR MB Bank. Nội dung chuyển khoản phải
                  là <strong>{user?.customer_code || "mã khách hàng"}</strong>.
                </p>
              </div>
              <button
                type="submit"
              ...
              <div className="wallet-history">
                {transactions.map((txn) => (
                  <div key={txn.id} className="wallet-history-item">
                    <div>
                      <p className="wallet-history-amount">
                        {formatCurrency(txn.amount)}
                      </p>
                      <p className="wallet-history-note">
                        {txn.note || "Nạp tiền ví"}
                      </p>
                    </div>
        ...
        {qrModal && pendingTopup && (
          ...
          >
            <div
              className="modal-content wallet-qr-modal"
              onClick={(e) => e.stopPropagation()}
            >
              <h2>Quét QR để nạp tiền</h2>
              <p>
                Số tiền: <strong>{formatCurrency(pendingTopup.amount)}</strong>
              </p>
              <p>
                Nội dung chuyển khoản (ID khách hàng):{" "}
                <strong className="wallet-code">{user?.customer_code}</strong>
              </p>
              {qrUrl && (
