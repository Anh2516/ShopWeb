...
const Users = () => {
  ...
  const openWalletModal = async (user) => {
    ...
    try {
      ...
      alert(
        error.response?.data?.message || "Không thể tải giao dịch chờ duyệt"
      );
  ...
  const handleApproveTopup = async (transactionId) => {
    try {
      const response = await axios.post(
        `/api/wallet/admin/${transactionId}/approve`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
        ...
        setUsers((prevUsers) =>
          prevUsers.map((u) => {
            if (u.id === walletModalUser?.id) {
              return {
                ...u,
                pending_topup_count: Math.max(
                  0,
                  (u.pending_topup_count || 0) - 1
                ),
              };
            }
            return u;
      ...
      window.dispatchEvent(
        new CustomEvent("pendingCountsUpdate", { detail: { immediate: true } })
      );
      await fetchUsers(debouncedSearch);
  ...
  const handleRejectTopup = async (transactionId) => {
    ...
    try {
      await axios.post(
        `/api/wallet/admin/${transactionId}/reject`,
        { reason },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
              ...
              <form onSubmit={handleSubmit}>
                ...
                {editingUser && (
                  <div className="form-group">
                    ...
                    <input
                      type="text"
                      value={editingUser.customer_code}
                      disabled
                    />
                  </div>
                )}
                      ...
                      {walletTransactions.map((txn) => (
                        <tr key={txn.id}>
                          ...
                          <td>
                            <button
                              className="btn btn-primary"
                              onClick={() => handleApproveTopup(txn.id)}
                            >
                              Duyệt
                            </button>
                            <button
                              className="btn btn-danger"
                              onClick={() => handleRejectTopup(txn.id)}
                            >
                              Từ chối
                            </button>
                          </td>
